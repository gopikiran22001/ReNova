    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const wasteRecycled = completedPickups.length;

                // Find next pending pickup
                const pendingPickups = pickups.filter(p => p.status === 'pending' || p.status === 'assigned')
                    .sort((a, b) => new Date(a.date) - new Date(b.date));
                const nextPickup = pendingPickups.length > 0 ? pendingPickups[0] : null;

                // Fetch user data for points
                const { data: userResponse } = await api.get('/auth/me');
                const userData = userResponse.success ? userResponse.data.user : user;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="card p-6">
                    <h3 className="text-gray-500 text-sm font-medium mb-1">Pickups Completed</h3>
                    <p className="text-3xl font-bold text-gray-900">{stats.wasteRecycled}</p>
                ...
                <div className="card p-6">
                    ...
                    {stats.nextPickup ? (
                        <>
                            <p className="text-xl font-bold text-gray-900">{new Date(stats.nextPickup.date).toLocaleDateString()}</p>
                            <p className="text-sm text-gray-500">{stats.nextPickup.wasteType}</p>
                        </>
                    ) : (
